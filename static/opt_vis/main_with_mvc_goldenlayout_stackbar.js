...
myLayout.registerComponent('highlightList', function (container, componentState){
...
myLayout.registerComponent('left', function (container, componentState){
...
myLayout.registerComponent('middle', function (container, componentState){
...
myLayout.registerComponent('right', function (container, componentState){
...
var makeCFGGraphView = function(model, svgId, divId) {
  ...
  var stackBarData = [];
  var stackBarArray = [];
  var keys = ['vector', 'memread', 'memwrite', 'call', 'syscall', 'fp'];

  var _prepareStackBarData = function(){
    // keys for the bar chart
    // vector, memread, memwrite, call, syscall, fp
 
    d3.selectAll("g.node.enter")
    .each(function(d) { 
      // get random values for each of the key 
      var thisBarData = {nodeId: d};
      var total = 0;
      var rands = [];
      for(var i=0; i<keys.length; i++){
        var temp_rand = Math.floor(Math.random()*100);
        rands.push(temp_rand);
        total += temp_rand;
      }

      var this_rect_arr = [];
      var this_cumulative = 0;

      // Normalize so the total is 1
      for (var i=0 ; i<keys.length; i++){
      
        thisBarData[keys[i]] = Math.round((rands[i]/total)*100);

        this_rect_arr.push([this_cumulative, this_cumulative + Math.round((rands[i]/total)*100)]);
        this_cumulative = this_cumulative + Math.round((rands[i]/total)*100);
      }

      var this_rect = d3.select(this).select("rect");
      thisBarData.rectX = parseFloat(this_rect.attr("x"));
      thisBarData.rectY = parseFloat(this_rect.attr("y"))

      stackBarData.push(thisBarData);
      
      stackBarArray.push(this_rect_arr);

    });

    console.log(stackBarData);
    console.log(stackBarArray);
  };
  ...
  var _render = function(){
    ...
    d3.selectAll("g.node.enter")
      ...
      _prepareStackBarData();

      /***** Edit code for stacked bar chart starts here ****/
      
      var stack_bar_width = 200;
      var stack_bar_height = 10;
      
      // var x = d3.scaleLinear()    // y = d3.scaleLinear()
      var x = d3.scale.linear()
          .rangeRound([0, stack_bar_width])  // .rangeRound([height, 0]);
          .domain([0, 100]); // values sum to 100 in this synthetic dataset
      ...
      // z.domain(keys);
      z.domain(d3.range(keys.length));

      /*
      // console.log(d3.layout.stack(stackBarData));
      console.log(d3.stack().keys(keys)(stackBarData));

      d3.selectAll("g.node.enter g")
      .data(d3.stack().keys(keys)(stackBarData))
      .enter().append("g")
      ...
      .data(function(d) { 
        ...
        return d; })
      .enter().append("rect")
        .attr("y", function(d) { return (d.data.rectY - stack_bar_height -3); })     //.attr("x", function(d) { return x(d.data.State); })
        .attr("x", function(d) { return x(d[0]) + d.data.rectX; })         //.attr("y", function(d) { return y(d[1]); }) 
        .attr("width", function(d) { return x(d[1]) - x(d[0]); }) //.attr("height", function(d) { return y(d[0]) - y(d[1]); })
        .attr("height", stack_bar_height);               //.attr("width", x.bandwidth());  
    */

    var node_height = 35; // Hardcoded for now
    var node_width = 270; // Hardcoded for now
    ...
    d3.selectAll("g.node.enter")
      .each(function(d,i){
        d3.select(this).append("g")
          .classed("stack", true)
          .selectAll("rect")
          .data(stackBarArray[i])
        .enter().append("rect")
          .style("fill", function(d,i) {return z(i);})
          .attr("y", function(d){return -node_height/2.0 - stack_bar_height - 5; })
          .attr("x", function(d) {return -node_width/2.0 + x(d[0]); })
          .attr("width", function(d) {return x(d[1]) - x(d[0]); })
          .attr("height", stack_bar_height);
      })
