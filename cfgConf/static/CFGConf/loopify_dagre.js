// NOTE: Upgraded to the new graphlib api
// Works on the leaf nodes (i.e. non function or loop node)

//This module runs after loop detector has run or after the loops are retrieved
// Uses the loopsObj to modify the dot file to show a semantic representation 
// of loops
var loopify_dagre = (function(myExport){
  ...
  // The data structure that contains the loop information
	var loopsObj;
  ...
  myExport.init = function(dotFile, filteredGraph){
    initVariables(dotFile, filteredGraph);
	  modifyDotGraph();

  };

  myExport.addBackground = function(bgId, graphContainerId, ndsAll, edgsAll){
    highlightBackEdges(edgsAll, loopsObj);
    renderBgFill(bgId, graphContainerId, ndsAll, edgsAll);
  }

  function initVariables(dF, filteredGraph){  
		modifiedDotFile = "";
		currRootLoop = null;
    ...
    dotFile = dF;
    loopsObj = getLoopifyTree(filteredGraph);
    ...
    // flipDir = !flipDir;
  		...
  		//exitNodes instead of a single loop.
	...
	// Populates the 'roots' array, and 'childrenForLoopBg' field for each of the loop 
  ...
  function modifyRecursively(index){
    ...
    for(var i=0; i<latches.length; i++){
          ...
          //EDIT: var port = (i%2) ? "e":"w";
          var port = "e";
          // EDIT: if(flipDir) { if(port==="e") {port="w";} else {port="e";} }
          // EDIT: if(isDoWhile) { 
            // EDIT: port = "w";
            // EDIT: port = (i%2) ? "w":"e"; 
...
// clear the backedge highlighting for all edges
function clearBackedges(edgesAll){
  for(let key in edgesAll){
    if(edgesAll.hasOwnProperty(key)){
      edgesAll[key].classed("backedge", false);
    }
  }
}

function highlightBackEdges(edgesAll, loopsObj){
  // clear the backedge highlighting for all edges
  clearBackedges(edgesAll);
  for(let loop of loopsObj){
    let header = loop.backedge[1];
    let tail = loop.backedge[0];
    let backedge = edgesAll[tail + "->" + header];
    backedge.classed("backedge", true);
  }
} 

function renderBgFill(bgId, graphContainerId, ndsAll, edgsAll){

  nodesAll = ndsAll;
  edgesAll = edgsAll;

  // Add a g as a top child inside "svg"
  d3.select("#" + bgId).remove();
  bgFillG = d3.select("#" + graphContainerId + " g").append("g").attr("id", bgId);
  // bgFillG.node().parentNode.insertBefore(bgFillG.node(), bgFillG.node().parentNode.firstChild);
  bgFillG.node().parentNode.insertBefore(bgFillG.node(), d3.select("#" + graphContainerId + " g.graph").select("polygon").node().nextSibling);
  ...
  // NOTE: This function is not used. So this function is not updated to the
  // new graphlib api and the graphviz svg format
...
function constructBgCurve(index, bgFillG){
  ...
  var backedge = edgesAll[edgeId.v + "->" + edgeId.w];
  var backedgeDattr = backedge.select("path").attr("d");

  // get the endpoints of the edge
  var first_re = /M(-?\d+(\.\d+)?)\s*,\s*(-?\d+(\.\d+)?)/;
  // var last_re = /[\s\S]*L(-?\d+(\.\d+)?)\s*,\s*(-?\d+(\.\d+)?)/;
  // Graphviz svg has different format 
  var last_re = /(-?\d+(\.\d+)?)\s*,\s*(-?\d+(\.\d+)?)$/;
  ...
  for(var k = 0; k<thisNodes.length; k++){
    ...
    var thisPoints = getPolygonPoints(thisNode);
...
//         {x: xcoord, y: ycoord} 
function getPolygonPoints(nodeId){
  let thisNode = nodesAll[nodeId];
  // let translate = d3.transform(thisNode.attr("transform")).translate;
  let ptStr = thisNode.select("polygon").attr("points");
  let pts = ptStr.split(" ");
  let points = [];
  for(let i=0; i<4; i++){
...
// NOTE: Modify this color map to handle greater nesting level. 
// Currently it handles upto depth 8.
function getBgColor(depth){
  ...
  // Colorbrewer colors 
  var colors = ['#fee6ce','#fdd0a2','#fdae6b','#fd8d3c','#f16913','#d94801','#a63603','#7f2704'];

  if(depth >= colors.length){
    depth = colors.length - 1;      
  }
  return colors[depth];
}

// u & v are node Ids
function addInvisEdgeOld(u, v) {
...
function addInvisEdge(v,w){
  if(!modified_graph.hasEdge(v,w)){
    modified_graph.setEdge(v, w, {style:"invis"});
  }
} 

//Use the headport and tailport attributes instead of the : syntax for ports
function modifyEdgeOld(u, v, port){
  ...
  modifiedDotFile = modifiedDotFile.replace(re, u + " -> " + v + "[headport=" + port + ", tailport=" + port + "]");
}

function modifyEdge(v, w, port){
  let thisEdge = modified_graph.edge(v,w);
  //TODO: BUG: Figure out why some of the edges are undefined 
  if(thisEdge){
    thisEdge["headport"] = port;
    thisEdge["tailport"] = port;
  }
}
  ...
  function getLoopifyTree(filteredGraph){

    let loopObjs = [];
    let loopIdArr;

	// Get all the loop type nodes with overlap type full
	// If "overlapType" flag not set, it means we are looking at an 
	// unfiltered graph. Get all loop type nodes in this case.
	loopIdArr = filteredGraph.nodes().filter(function(nodeId){
	  	let nodeObj = filteredGraph.node(nodeId);
	  	return (nodeObj["type"] == nodeTypes.loop) 
	    	&& (!("overlapType" in nodeObj) || nodeObj["overlapType"] == OverlapTypes.full);
	});

	for(let loopId of loopIdArr){
      let loop = filteredGraph.node(loopId);	
      for(let backedge of loop["backedges"]){
        let newLoop = {};
        // make a copy of each array to preserve the original 
        newLoop["backedge"] = [...backedge];
        newLoop["nodes"] = [...loop["nodes"]];
        newLoop["parent"] = filteredGraph.parent(loopId);
        newLoop["loopId"] = loopId;
        loopObjs.push(newLoop);
      }
    } 

    // Set the parent property for each of the loop to the index in the array 
    // or "" (if none) 
    for(let loop of loopObjs){
    	if(loop["parent"]){
      		if(filteredGraph.node(loop["parent"])["type"] == nodeTypes.loop){
        		loop["parent"] = _findFirstIndex(loop["parent"], loopObjs, "loopId");
      		}	else {
      			loop["parent"] = "";
      		}	
    	}
	...
	// Returns the first index in an array of objects with the given value 
	// for the key
	// Returns -1 if value not found
	function _findFirstIndex(val, arr, key){
